...
#                 raise ValueError("Not compatible with the number of unique tokens.")
...
#                 sequence = [tokens[i % num_unique_tokens] for i in range(max_sequence_size)]
#                 reward = random.choice([10, 20, 30])
#                 sequences.append((''.join(sequence), reward))
...
#         matrix = [file.readline().split() for _ in range(matrix_size[0])]
...
# def calculate_reward(path, sequences):
#     reward = 0
#     path_str = ''.join(path)
#     for sequence, reward_value in sequences:
#         found = KMPSearch(sequence, path_str)
#         if found != 0:
...
def file_input(filename):
    with open(filename, 'r') as file:
        buffer_size = int(file.readline())
        matrix_size = tuple(map(int, file.readline().split()))
        matrix = np.array([file.readline().split() for _ in range(matrix_size[0])])  # Menggunakan np.array
        num_of_sequences = int(file.readline())
        sequences = []
        for _ in range(num_of_sequences):
            sequence = ''.join(file.readline().split())
